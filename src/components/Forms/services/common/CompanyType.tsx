  ...
  return (
    <form className='w-full max-w-[650px]'>
      <div className=' flex flex-col items-center gap-4'>
        ...
        <div className='mt-3 flex max-w-screen-md flex-wrap justify-center gap-10'>
          {CompanyType?.map((item) => (
            <Link
              key={item.id}
              href={`/create?type=${item.name}&step=0`}
              className='border-darkgray-100 flex max-w-[150px] basis-48 cursor-pointer  flex-col flex-wrap items-center justify-center gap-[10px] overflow-hidden rounded-2xl border-[1px] border-solid bg-white px-5 py-[17px] transition-all duration-300 ease-in-out hover:border-orange-500'
            >
              <Image
